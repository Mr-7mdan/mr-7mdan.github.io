...
class _ListItem(object):
    ...
    def get_listitem(self, offscreen=True):
        ...
        # Set parental guide properties on the container level
        for key, value in self.infoproperties.items():
            if key.startswith('pg_'):
                listitem.setProperty(key, value)
        # logging.info(f"Set parental guide properties on container: {[k for k in self.infoproperties if k.startswith('pg_')]}")

        # Log all properties
        # logging.info("All properties set on listitem:")
        for key in self.infoproperties:
    ...
    def set_parental_guide(self, parental_guide):
        if parental_guide:
            for key, value in parental_guide.items():
                self.infoproperties[key] = str(value)  # Ensure the value is a string
