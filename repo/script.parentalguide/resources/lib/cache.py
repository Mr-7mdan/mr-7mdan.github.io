...
import sqlite3
import json
from .api_client import get_parental_guide
...
def cache_details(show_info):
    # type: (Dict[Text, Any]) -> None
    """
    Save show_info dict to cache
    """
    file_name = str(show_info['id']) + "-" +str(show_info['provider']) + '.pickle'
    cache = {
        'show_info': show_info
    }
...
def load_cache(show_id, provider):
    ...
    """
    file_name = str(show_id) + "-" +str(provider) + '.pickle'
    try:
...
def get_cached_or_fetch(imdb_id):
    conn = sqlite3.connect('parental_guide_cache.db')
    c = conn.cursor()
    
    # Check cache
    c.execute("SELECT data FROM cache WHERE imdb_id=? AND timestamp > datetime('now', '-1 day')", (imdb_id,))
    result = c.fetchone()
    ...
    # Cache the result
    c.execute("INSERT OR REPLACE INTO cache (imdb_id, data, timestamp) VALUES (?, ?, datetime('now'))",
              (imdb_id, json.dumps(data)))
    conn.commit()
    conn.close()
