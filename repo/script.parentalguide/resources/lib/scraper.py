...
class IMDBScraper(Scrapper):
    def parentsguide(IMDBID, videoName):
        # Prefer the server-side aggregator; fall back to direct IMDb GraphQL.
        from resources.lib.api_client import get_parental_guide
        review = get_parental_guide(imdb_id=IMDBID, video_name=videoName, provider="imdb")
        if review and review.get("review-items"):
            return review
