...
def get_imdb(params):
	...
	if action in ('imdb_watchlist', 'imdb_user_list_contents', 'imdb_keywords_list_contents'):
		...
		for item in results:
			...
			#addition
			try:
				ranking = replace_html_codes(parseDOM(item, 'span', attrs={'class': 'ipl-status-pill '})[0])
				# if ranking is not None : item_dict['ranking'] = ranking
				# if ranking != None : item_dict['ranking'] = ranking
				# if ranking is not "" : item_dict['ranking'] = ranking
				# if ranking is not " " : item_dict['ranking'] = ranking
				# if ranking !="" : item_dict['ranking'] = ranking
				# if ranking !=" " : item_dict['ranking'] = ranking
				if ranking in ['None','Mild','Moderate','Severe']: item_dict['ranking'] = ranking
				else: 			
					ranking = replace_html_codes(parseDOM(item, 'span', attrs={'class': 'ipl-status-pill ipl-status-pill--(.+?)'})[0])
					item_dict['ranking'] = ranking
			except: continue
			#votes
			try:
				votes = replace_html_codes(parseDOM(item, 'a', attrs={'class': 'advisory-severity-vote__message'})[0])
				substring = 'found this'
				if substring in votes:
					item_dict['votes'] = votes
			except: continue
