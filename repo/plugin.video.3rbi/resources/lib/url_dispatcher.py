...
from resources.lib.basics import addDir, addDownLink, addImgLink, searchDir, addon_image, addon
...
class URL_Dispatcher(object):
    ...
    def add_dir(self, name, url, mode, iconimage=None, page=None, channel=None, section=None, keyword='', Folder=True,
                about=None, custom=False, list_avail=True, listitem_id=None, custom_list=False, contextm=None, desc='', fanart=None, landscape=None, fav='add',
        ...
        if not self.widget or 'Next ' in name:
            addDir(name, url, mode, iconimage, page, channel, section, keyword, Folder, about,
                   custom, list_avail, listitem_id, custom_list, contextm, desc, fanart, landscape, fav=fav,
                   year=year, season=season, episode=episode, show_title=show_title, media_type=media_type, original_title=original_title)

    def add_download_link(self, name, url, mode, iconimage, desc='', stream=None, fav='add', noDownload=False, contextm=None, fanart=None, duration='', quality='', landscape=None,
                          year=None, season=None, episode=None, show_title=None, media_type=None, original_title=None, episode_name=None):
        mode = self.get_full_mode(mode)
        if filter_listing:
            bypass_list = filter_listing.split(';')
            if any(x.lower() in name.lower() for x in bypass_list):
                return
        addDownLink(name, url, mode, iconimage, desc, stream, fav, noDownload, contextm, fanart, duration, quality, landscape=landscape,
                    year=year, season=season, episode=episode, show_title=show_title, media_type=media_type, original_title=original_title, episode_name=episode_name)
