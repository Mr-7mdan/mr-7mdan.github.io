    ...
    try:
        if 'folders' in favorder:
            ...
            for (domain, mode, count) in c.fetchall():
                site = mode.split('.')[0]
                img = ''
                name = get_site_name(site, domain, sites_list)
                name = '{} [{} favorites]'.format(name, count)
            ...
            for (domain, name, url, mode, img, duration, quality) in c.fetchall():
                ...
                if 'site' in favorder:
                    sitename = get_site_name(site, domain, sites_list)
                    name = '[{}]{}'.format(sitename, name)

                is_video = any(x in mode for x in ['Play', 'Resolve'])
                if is_video:
                    basics.addDownLink(name, url, mode, img, desc='', stream='', fav='del', duration=duration, quality=quality)
                else:
                    basics.addDir(name, url, mode, img, Folder=True, list_avail=True, fav='del')
...
def get_site_name(site, domain, sites_list):
...
def FavListSite(url, page=1):
    ...
    try:
        ...
        if total_items > offset + items_per_page:
            basics.addDir('Next Page ({}/{})'.format(page + 1, pages), '{}@{}'.format(mode, domain), 'favorites.FavListSite', '', page=page + 1)
...
def backup_fav():
    ...
    backup_content = {"meta": {"type": "aksv-favorites", "version": 1, "datetime": time}, "data": favorites}
    if progress.iscanceled():
        progress.close()
        return
    progress.update(75, "Writing backup file")
    filename = "aksv-favorites_" + time + '.bak'
...
def restore_fav():
    ...
    if compressbackup:
        ...
        try:
            ...
            utils.notify("Error", "Invalid backup file")
            return
    if not backup_content["meta"]["type"] == "aksv-favorites":
...
def install_custom_site():
    if not utils.dialog.yesno('WARNING',
                              'Custom sites are not verified by 3rbi, and could contain malware.[CR]'
                              '3rbi is not responsible for custom sites. Proceed?'):
        return
    path = utils.dialog.browseSingle(1, 'Select 3rbi custom site file', 'myprograms')
    if not path:
        return
...
def load_custom_list(url):
    ...
    for (rowid, name, url, mode, img) in c.fetchall():
        if not img.startswith('http'):
            ...
            img = basics.addon_image(img, custom)
        ins = SiteBase.get_site_by_name(mode.split('.')[0])
        if ins:
...
def add_listitem(favmode, name, url, img):
    ...
    if selected_id:
        ...
        if c.fetchone()[0] > 0:
            if utils.dialog.yesno('Item already in list', 'This item is already in the list {}[CR]Update the name and image?'.format(selected_list)):
